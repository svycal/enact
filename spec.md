...
- **`Enact.Validations.unique/3`:** scoped uniqueness for input changesets (`scope:`, `repo:`, `query:`, `except:` opts); skips when the field has no change. `except:` excludes the current row on PATCH (struct, id, or keyword). Soft-deletes stay on `:query`.
